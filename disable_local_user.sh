...
usage() {
  ...
  echo '  -d  Deletes accounts instead if disabling them.' >&2
...
do
  ...
  if [[ "${USERID}" -lt 1000 ]]
  then
    echo "Refuising to remove the ${USERNAME} account with UID ${USERID}" >&2
    exit 1
  fi
  ...
  then
    ...
    then
      ...
      if [[ "${?}" -ne 0 ]]
      then
        echo "The archive directory ${ARCHIVE_DIR} could not be crated." >&2
        exit 1
      fi
    fi
